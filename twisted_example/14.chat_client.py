...
if __name__ == '__main__':
    ...
    if is_disconnect:
        reactor.callLater(2, cf.p.send_heartbeat)
    else:
        task_send_heartbeat = task.LoopingCall(cf.p.send_heartbeat)
        task_send_heartbeat.start(2, now = False)

    reactor.callLater(1, cf.p.send_verify, chat_from)
    reactor.callLater(5, cf.p.send_single_chat, chat_from, '000002', '你好,这是5秒的时候发送')
